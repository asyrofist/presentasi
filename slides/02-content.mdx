...
```python subtitle='function untuk menggunakan word2vec'
def word2vec():
...
```python subtitle='function untuk membuat tabel word2vec'
def tableWord2vec():
    tabelWord2vec = []
    for num in range(0, len(cleaned_text)):
      a = [modelWord2vec.wmdistance(cleaned_text[num].split(), cleaned_text[angka].split()) \ 
...
```python subtitle='class function untuk menghitung dengan WMD'
